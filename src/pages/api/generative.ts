...
export default async function handler(
    ...
    const {prompt, context, provider, instructedPromptEnabled, sensitivity} = req.body;

    const enhancedPrompt = [
        `You are given a set of documents about: "${context}".`,
        instructedPromptEnabled ? `you need to respond to user's prompt only using the information in the documents.` : "",
